...
import io.github.sakuraryoko.corelib.network.payload.DataPayload;
import io.github.sakuraryoko.corelib.network.payload.StringPayload;
...
public class C2SHandler {
    ...
    public static void receive(DataPayload payload, ClientPlayNetworking.Context context) {
        CoreLog.info("C2SHandler#receive() B: received C2SData Payload (size in bytes): "+ payload.data().getSizeInBytes());
        ...
        CoreLog.info("C2SHandler#receive() B: buf size in bytes: "+buf.readableBytes());
        String response = buf.readString();
        CoreLog.info("C2SHandler#receive() B: String: "+response);
    }
